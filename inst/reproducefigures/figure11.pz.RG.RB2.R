...
estimates <- foreach(irep = 1:nrep, .combine = rbind) %dorng% {
  res <- rheeglynn_estimator_RB2(observations, pz, theta, algoparameters)
  data.frame(irep = irep, time = 0:datalength, estimate = t(res$estimate), iteration = res$iteration, meetingtime = res$meetingtime)
}

filename <- paste0("pz.RB2.R", nrep, "N", nparticles, "T", datalength, ".RData")
